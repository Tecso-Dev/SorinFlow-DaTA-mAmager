...
STATUS_FA = {
    ...
    412: ("شماره فرستنده نامعتبر است یا اجازهٔ ارسال به این مقصد را ندارد — "
          ...
          "کد ورود از راه «الگو» می‌رود و به خط فرستنده نیازی ندارد، اما "
          "ارسال تکی و گروهی حتماً به یک خط داخلی ایران نیاز دارد — از بخش "
          "«خطوط» پنل کاوه‌نگار یک خط داخلی تهیه کنید."),
    ...
    # Also what a template still «در حال بررسی» answers. Kavenegar does not
    # distinguish "never existed" from "not approved yet", and the bare wording
    # sends someone hunting for a typo in a name that is spelled correctly.
    424: ("الگوی مورد نظر یافت نشد — اگر تازه ساخته‌اید، هنوز در صف بررسی "
          "کاوه‌نگار است و تا تأیید نشدن کار نمی‌کند."),
