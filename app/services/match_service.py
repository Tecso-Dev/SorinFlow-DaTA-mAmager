...
def score_similarity(target: Property, cand: Property) -> Dict[str, Any]:
    ...
    if target.property_type and cand.property_type:
        parts.append((10, 1.0 if target.property_type == cand.property_type else 0.0, "همان نوع ملک"))
    ...
    score = sum(w * v for w, v, _r in parts) / total_w * 100 * price_penalty
...
def score_for_customer(customer, cand: Property) -> Dict[str, Any]:
    ...
    total_w = sum(w for w, _v, _r in parts) or 1
    score = sum(w * v for w, v, _r in parts) / total_w * 100
    ...
    if use_llm and results:
        ...
        reasons = await _llm_rerank(
            [{"id": r["id"], "title": r["title"], "area": r["area"], "rooms": r["rooms"],
              "price": r["price"], "district": r["district"], "city": r["city_name"]} for r in results],
            ctx,
        )
        for r in results:
...
async def matches_for_customer(db: AsyncSession, customer, limit: int = 12,
    ...
    # a generous budget ceiling keeps the pool small without hiding near-misses
    cands = (await db.execute(q.limit(CANDIDATE_POOL * 2))).scalars().all()
