...
_DASHBOARD_CACHE_KEY = "stats:dashboard"
...
async def get_dashboard_stats(
    ...
    """Get dashboard statistics (cached 60s in Redis)."""
    redis = await get_redis()
    cached = await redis.get(_DASHBOARD_CACHE_KEY)
    if cached:
    ...
    await redis.setex(_DASHBOARD_CACHE_KEY, _DASHBOARD_CACHE_TTL, result.model_dump_json())
    return result
